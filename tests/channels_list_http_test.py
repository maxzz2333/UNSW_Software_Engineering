...
def test_list_invalid_token():
    ...
    list_response = requests.get(f"{config.url}channels/list/v2", params={'token': 'invalidtoken'})
    assert list_response.status_code == ACCESS_ERROR_CODE

def test_list_invalid_user():
    clear_response = requests.delete(f"{config.url}clear/v1")
    assert clear_response.status_code == SUCCESS_CODE

    list_response = requests.get(f"{config.url}channels/list/v2", params={'token': generate_jwt(99)})
    assert list_response.status_code == ACCESS_ERROR_CODE
...
def test_list_simple():
    ...
    list_response = requests.get(f"{config.url}channels/list/v2", params={'token': user1_token})
