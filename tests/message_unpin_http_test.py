...
def test_message_unpin_token_invalid():
    #clear data_store
    ...
    message_unpin_response = requests.post(f"{config.url}message/unpin/v1",json={'token': 1, 'message_id': message_id})
    assert message_unpin_response.status_code == ACCESS_ERROR_CODE

def test_message_unpin_authuser_invalid():
    #clear data_store
    response = requests.delete(f"{config.url}clear/v1")
    assert response.status_code == SUCCESS_CODE
    ...
    #unpin message in channel
    message_unpin_response = requests.post(f"{config.url}message/unpin/v1",json={'token': generate_jwt(user1_id, 10), 'message_id': message_id})
    assert message_unpin_response.status_code == ACCESS_ERROR_CODE

#Occurrences of InputError message_id is not a valid message within a channel or DM that the authorised user has joined

def test_message_unpin_message_invalid():
    #clear data_store
    response = requests.delete(f"{config.url}clear/v1")
    assert response.status_code == SUCCESS_CODE
    ...
    #unpin message in channel
    message_unpin_response = requests.post(f"{config.url}message/unpin/v1",json={'token': user1_token, 'message_id': -1})
    assert message_unpin_response.status_code == INPUT_ERROR_CODE

def test_message_unpin_message_already_unpinned_in_channel():
    #clear data_store
    response = requests.delete(f"{config.url}clear/v1")
    assert response.status_code == SUCCESS_CODE
    ...
    #unpin message in channel
    message_unpin_response = requests.post(f"{config.url}message/unpin/v1",json={'token': user1_token, 'message_id': message_id})
    assert message_unpin_response.status_code == SUCCESS_CODE
...
def test_message_unpin_message_already_unpinned_in_DM():
    #clear data_store
    response = requests.delete(f"{config.url}clear/v1")
    assert response.status_code == SUCCESS_CODE
    ...
    #unpin message in DM
    message_unpin_response = requests.post(f"{config.url}message/unpin/v1",json={'token': user_1['token'], 'message_id': message_id})
    assert message_unpin_response.status_code == SUCCESS_CODE
...
def test_message_unpin_message_valid_user_not_owner_permissions_in_channel():
    #clear data_store
    response = requests.delete(f"{config.url}clear/v1")
    assert response.status_code == SUCCESS_CODE
    ...
    #unpin message in channel
    message_unpin_response = requests.post(f"{config.url}message/unpin/v1",json={'token': user3_token, 'message_id': message_id})
    assert message_unpin_response.status_code == ACCESS_ERROR_CODE
...
def test_message_unpin_message_valid_user_not_owner_permissions_in_channel_user_is_golber_owner():
    #clear data_store
    response = requests.delete(f"{config.url}clear/v1")
    assert response.status_code == SUCCESS_CODE
    ...
    #pin message in channel
    message_pin_response = requests.post(f"{config.url}message/pin/v1",json={'token': user2_token, 'message_id': message_id})
    assert message_pin_response.status_code == SUCCESS_CODE
...
def test_message_unpin_message_valid_user_not_owner_permissions_in_DM():
    #clear data_store
    response = requests.delete(f"{config.url}clear/v1")
    assert response.status_code == SUCCESS_CODE
    ...
    messages_0 = dm_messages_detail[0]
    assert messages_0['message_id'] == message_id
    assert messages_0['u_id'] == user_1['auth_user_id']
    assert messages_0['is_pinned'] == True
    

    #unpin message in DM
    message_unpin_response = requests.post(f"{config.url}message/unpin/v1",json={'token': user_2['token'], 'message_id': message_id})
    assert message_unpin_response.status_code == ACCESS_ERROR_CODE
...
def test_message_unpin_message_valid_user_has_owner_permissions_in_channel():
    #clear data_store
    response = requests.delete(f"{config.url}clear/v1")
    assert response.status_code == SUCCESS_CODE
    ...
    messages_1 = channel_messages_detail[1]
    assert messages_1['message_id'] == message_id
    assert messages_1['is_pinned'] == False

def test_message_unpin_message_valid_user_has_owner_permissions_in_channel_2():
    #clear data_store
    response = requests.delete(f"{config.url}clear/v1")
    assert response.status_code == SUCCESS_CODE
...
def test_message_unpin_message_valid_user_has_owner_permissions_in_channel_user_is_golber_owner():
    #clear data_store
    response = requests.delete(f"{config.url}clear/v1")
    assert response.status_code == SUCCESS_CODE
...
def test_message_unpin_message_valid_user_has_owner_permissions_in_dm():
    #clear data_store
    response = requests.delete(f"{config.url}clear/v1")
    assert response.status_code == SUCCESS_CODE
...
def test_message_unpin_message_valid_user_has_owner_permissions_in_dm_2():
    #clear data_store
    response = requests.delete(f"{config.url}clear/v1")
    assert response.status_code == SUCCESS_CODE
